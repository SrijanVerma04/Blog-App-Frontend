...
import {AppBar, Button, Toolbar, Box, Tabs, Tab, Typography} from "@mui/material"
          ...
          { isLoggedIn &&  <Box display={"flex"} marginLeft={"auto"} marginRight={"auto"} >
            ...
            <Box display="flex" marginLeft='auto'>
               { !isLoggedIn && <> <Button LinkComponent={Link} to="/auth" variant='contained' sx={{margin: 1, borderRadius: 10}} color='warning'> Login  </Button>
                ...
                { isLoggedIn && <Button LinkComponent={Link} to="/auth" onClick={() => dispatch(authActions.logout())} variant='contained' sx={{margin: 1, borderRadius: 10}} color='warning'> Logout </Button> }
            </Box>
        </Toolbar>
    </AppBar>
  )
